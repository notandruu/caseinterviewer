...
export function VoiceSessionV2({ caseData, interviewId, userId }: VoiceSessionProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Only check for silence if user has started speaking (has interim transcript)
  ...
  }, [sessionState, isRecording, currentTranscript])
    ...
    <div className="vs-screen">
      ...
      <div style={{ position: 'absolute', top: '2rem', right: '2rem', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
        ...
        <button
          ...
          style={{
            padding: '0.5rem 1rem',
            backgroundColor: textModeEnabled ? '#2196F3' : 'transparent',
            border: `1px solid ${textModeEnabled ? '#2196F3' : '#555'}`,
            borderRadius: '0.5rem',
            color: textModeEnabled ? '#ffffff' : '#555',
            cursor: 'pointer',
            fontSize: '0.75rem',
            fontWeight: 500,
            transition: 'all 200ms ease',
