...
export function VoiceInterviewClient({ caseData, interviewId, userId }: VoiceInterviewClientProps) {
  ...
  useEffect(() => {
    ...
    if (SR) {
      ...
      r.onerror = (e: any) => {
        console.error("[CaseNow] Speech recognition error:", e?.error);
  ...
  const handleAIResponse = async (userInput: string) => {
    try {
      ...
      console.error("[CaseNow] AI response error:", e);
    }
  };
  ...
  const speakText = async (text: string) => {
    ...
    try {
      ...
      console.warn("[CaseNow] TTS not audio:", await r.text());
      playLocal(text);
    } catch (e) {
      console.error("[CaseNow] TTS fetch failed:", e);
      playLocal(text);
    }
  };
  ...
  const startInterview = async () => {
    ...
    try {
      ...
      console.error("[CaseNow] Error starting interview:", e);
    }
